...
const usFmtFraction = function(arr) {
...
const aggregatorTemplates = {
  ...
  countExperiments(formatter = usFmtInt) {
    return () => function () {
      return {
        count: 0,
        push(record) {
    ...
    return function ([attr]) {
      return function () {
        return {
          ...
          format: formatter,
          numInputs: typeof attr !== 'undefined'
            ? 0
            : 1
        };
      };
  ...
  countWithState(formatter = usFmtFraction) {
    return function ([attr]) {
      return function () {
        return {
          selectedCount: {selected: [], notselected: []},
          push(record) {
            
            if (record.selected) {
              this.selectedCount.selected.push(record);
            } else {
              this.selectedCount.notselected.push(record);
            }
          },
          value() {
            return [this.selectedCount.selected.length, this.selectedCount.notselected.length];
          },
          format: formatter,
          numInputs: typeof attr !== 'undefined'
            ? 0
  ...
  extremes(mode, formatter = usFmt) {
    return function ([attr]) {
      return function (data) {
        return {
          val: null,
          sorter: getSort(typeof data !== 'undefined'
            ? data.sorters
            : null, attr),
          push(record) {
            let x = record[attr];
            if (['min', 'max'].includes(mode)) {
              x = parseFloat(x);
              if (!isNaN(x)) {
                this.val = Math[mode](x, this.val !== null
                  ? this.val
                  : x);
              }
            }
            if (mode === 'first' && this.sorter(x, this.val !== null
              ? this.val
              : x) <= 0) {
              this.val = x;
            }
            if (mode === 'last' && this.sorter(x, this.val !== null
              ? this.val
              : x) >= 0) {
              this.val = x;
            }
    ...
    return function ([num, denom]) {
      return function () {
        return {
          ...
          format: formatter,
          numInputs: typeof num !== 'undefined' && typeof denom !== 'undefined'
            ? 0
            : 2
        };
      };
...
const aggregators = (tpl => ({
  ...
  'Count Experiments': tpl.countExperiments(usFmtInt),
  'Count Values with State': tpl.countWithState(usFmtFraction),
...
*/
  ...
  class PivotData {
      ...
      if (!this.sorted) {
        ...
        switch (this.props.rowOrder) {
          ...
          default:
            this
              .rowKeys
              .sort(this.arrSort(this.props.rows));
        }
        switch (this.props.colOrder) {
          ...
          default:
            this
              .colKeys
              .sort(this.arrSort(this.props.cols));
        }
      }
    }
    ...
    processRecord(record) {
      ...
      for (const x of Array.from(this.props.cols)) {
        colKey.push(x in record
          ? record[x]
          : 'null');
      }
      for (const x of Array.from(this.props.rows)) {
        rowKey.push(x in record
          ? record[x]
          : 'null');
      }
  ...
  PivotData.defaultProps = {
    ...
    aggregatorName: 'Count Experiments',
    sorters: {},
    valueFilter: {},
    rowOrder: 'key_a_to_z',
    colOrder: 'key_a_to_z',
  ...
  PivotData.propTypes = {
    data: PropTypes
      .oneOfType([PropTypes.array, PropTypes.object, PropTypes.func])
      .isRequired,
    ...
    sorters: PropTypes.oneOfType([
      PropTypes.func, PropTypes.objectOf(PropTypes.func)
    ]),
